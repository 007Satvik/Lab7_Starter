...
function initializeServiceWorker() {
  ...
  if("serviceWorker" in navigator ){
    window.addEventListener('load', async event1 => {
      try {
        const registration = await navigator.serviceWorker.register("./sw.js", {scope:'./' }).then((registration) =>
        console.log("Success"));
      }
      catch (error){
        console.log('Failed');
...
async function getRecipes() {
  ...
  if(window.localStorage.length != 0)
   return locrecipe = JSON.parse(window.localStorage.getItem('recipes'));
  //
  //alert(locrecipe);
  /**************************/
  ...
  var promise = new Promise(async (resolve, reject) => {
    for(let i = 0; i < RECIPE_URLS.length; i++){
      try{
        const temp = await fetch(RECIPE_URLS[i]);
        const rec = await temp.json();
        console.log(rec);
        locArray.push(rec);
        if(i == RECIPE_URLS.length - 1){
            saveRecipesToStorage(locArray);
            resolve(locArray);
        }
      }
      catch (error){
